...
(function (window) {
    ...
    (function () {
        ...
        function TVar(id, obj, stm) {
            ...
            this.proxied = this.createProxy(this.handler, obj);
        }
                ...
                handler = {
                    ...
                    getPropertyDescriptor: function (name) {
                        ...
                        while (obj !== undefined && obj !== null) {
                            ...
                            obj = tvar.proxied;
                            desc = tvar.handler.getOwnPropertyDescriptor(null, name);
                            if (undefined === desc) {
                                visited.push(tvar);
                    ...
                    defineProperty: function (name, property, desc) {
                    	if(desc.get || desc.set){
                    		Object.defineProperty(name, property, desc);
                    		return true;
                    	}
                        ...
                        if (stm.inTransaction()) {
                            /*if (util.hasOwnProp.call(desc, 'get') ||
                                ...
                                current = handler.getOwnPropertyDescriptor(null, name);
                    ...
                    erase: function (name) {
                        ...
                        if ("_map" === name) {
                            ...
                            desc = handler.getOwnPropertyDescriptor(null, name);
                            if (undefined === desc) {
                                return true;
                    ...
                    hasOwn: function (name) {
                        ...
                        if ("_map" === name || ! stm.inTransaction()) {
                            return !!handler.getOwnPropertyDescriptor(null, name);
                        } else if (stm.transactionFrame.isDeleted(self, name)) {
                            return false;
                        } else {
                            desc = stm.transactionFrame.get(self, name);
                    ...
                    get: function (receiver, name, three) {
                        self.log("get:", name);
                        
                        if((Array.isArray(receiver) && (name === "length")) ||
                        		(typeof name !== "symbol") && (name !== "_isVue") && ((receiver[name] === undefined) || receiver.propertyIsEnumerable(name))){
                    ...
                    for (j = 0; j < names.length; j += 1) {
                        ...
                        if (this.deleted === value) {
                            ...
                            if (tvar.isArray && 'length' === name) {
                            	...
                            	if(tvar.raw.length > value.value){
                            		tvar.raw.splice(value.value, 1);
                            	}
                            	else{
                            		tvar.raw[name] = value.value;
                            	}
                            	...
                            	if(tvar.isArray) {
        	                    	if(tvar.raw[name] === undefined){
        	                    		tvar.raw.push(value.value);
        	                    	}
        	                    	else{
        	                    		tvar.raw.splice(parseInt(name, 10), 1, value.value);
        	                    	}
                            	}
                            	else{
                            		tvar.raw[name] = value.value;
            ...
            applyUpdate: function (tvar, name, desc, updates) {
                ...
                if (! util.hasOwnProp.call(desc, 'tvar')) {
                    if (tvar.isArray && 'length' === name) {
                    	...
                    	if(tvar.raw.length > desc.value){
                    		tvar.raw.splice(desc.value, 1);
                    	}
                    	else{
                    		tvar.raw[name] = desc.value;
                    	}
                    	...
                    	if(tvar.isArray) {
	                    	if(tvar.raw[name] === undefined){
	                    		tvar.raw.push(desc.value);
	                    	}
	                    	else{
	                    		tvar.raw.splice(parseInt(name, 10), 1, desc.value);
	                    	}
                    	}
                    	else{
                    		tvar.raw[name] = desc.value;
